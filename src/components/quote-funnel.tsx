...
export function QuoteFunnel({ initialData, onExit }: QuoteFunnelProps) {
  ...
  const renderStepContent = () => {
    if (isTransitioning) return <div className="min-h-[200px] flex items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary"></div></div>
        ...
        return (
            <div className="text-center space-y-4">
                ...
                <p>We will also send a confirmation via SMS to <strong>{formData.phone}</strong>.</p>
                 <Button size="lg" onClick={onExit}>Return to Homepage</Button>
            </div>
        );
      ...
      <div className="container mx-auto px-4 md:px-6">
        <Card className="mx-auto max-w-2xl shadow-lg">
          <CardHeader className="text-center relative">
             {step > 0 && (
                <Button variant="ghost" onClick={handleBack} className="absolute left-4 top-1/2 -translate-y-1/2">
